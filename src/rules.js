...
/* 送球が捕手から始まる理由。盗塁アウトは捕手の送球で刺すため、
   送球の順の初期値に捕手を入れておく。挟殺（ランダウン）は投手や内野手から
...
/* 投球と同時に起きる走者のプレー。紙は必ずその1球も書く。
   ...
   ボークを入れているのは、全軟連の「学童部(小学生)の投球数制限について」（2020年、
   競技者必携改正）が「ボークにかかわらず投球したものは、投球数に数える」と
   定めているため。投球を伴わないボークもあるので、数えるかどうかはその場で選ぶ。
   けん制は同じ文書の「牽制球や送球とみなされるものは投球数としない」ため入れない。

   盗塁・盗塁失敗・暴投・捕逸は条文に名指しがない。いずれも投球を伴うプレーなので、
...
/* 学童部の投球数制限。競技に関する連盟特別規則《学童部（女子共）》
   ...
   同 ④ で2026年シーズンから1週間210球以内（4年生以下180球以内）も加わったが、
   試合をまたいだ集計はできないため出していない（FR-15／FR-16 が前提）。

   学年はメンバー表に書かないため、選手マスタから拾う。分からないときは
   上限を出さない。70球と決め打つと、4年生以下の選手で10球ぶん甘くなる */
export const GRADES = [1, 2, 3, 4, 5, 6];
export const pitchLimit = (grade) => (grade == null ? null : grade <= 4 ? 60 : 70);
...
export function battedKind(result, batted) {
  if (GROUND.has(result)) return "ground";
  if (FLY.has(result)) return "fly";
  if (result === "ライナーアウト") return "liner";
  if (batted === "ゴロ") return "ground";
  if (batted === "フライ") return "fly";
  if (batted === "ライナー") return "liner";
  return "";
}

/** 1マスぶんの記号を作る */
function resultMark(e) {
  const seq = fieldersNotation(fieldersOf(e)) || zoneNotation(e.zone, e.zone2);
